...
async function submitEvidence(body) {
  ...
  try {
    ...
    const manifest = {requestId: body.requestId, evidenceId: body.evidenceId, source: body.source, endpoint: body.endpoint, nonce: body.nonce, policyVersion: body.policyVersion, rawSourceHash: body.rawPayloadDigest, transformedHash: body.transformedPayloadDigest, tlsProofType: body.tlsProofType, tlsProofHash: body.tlsProofHash, canonicalizationVersion: body.canonicalizationVersion, attestedPublicKeyBase64: body.attestedPublicKeyBase64};
    ...
    const attestedKey = crypto.createPublicKey({key: Buffer.from(body.attestedPublicKeyBase64, 'base64'), format: 'der', type: 'spki'});
    if (!crypto.verify('sha256', Buffer.from(canonicalize(manifest)), attestedKey, Buffer.from(body.enclaveSignature, 'base64'))) throw new Error('ENCLAVE_SIGNATURE_INVALID');
