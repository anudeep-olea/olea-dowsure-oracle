...
function verifyCoseSign1(signatureValue, certificate) {
  ...
  verifier.end();
  const signatureValue = algorithmSpec.ecdsa ? signatureToDer(signature) : signature;
  ...
  if (!verifier.verify(verifyOptions, signatureValue)) throw new Error('ATTESTATION_COSE_SIGNATURE_INVALID');
