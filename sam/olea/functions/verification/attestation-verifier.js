...
class CborReader {
  ...
  read() {
    ...
    if (additional === 31) return this.indefinite(major);
  ...
  length(additional) {
    ...
    throw new Error('ATTESTATION_CBOR_LENGTH_INVALID');
  }

  indefinite(major) {
    if (major === 2 || major === 3) {
      const chunks = [];
      while (this.buffer[this.offset] !== 0xff) chunks.push(this.read());
      this.offset += 1;
      if (major === 2 && chunks.every(Buffer.isBuffer)) return Buffer.concat(chunks);
      if (major === 3 && chunks.every((chunk) => typeof chunk === 'string')) return chunks.join('');
      throw new Error('ATTESTATION_CBOR_INDEFINITE_CHUNK_INVALID');
    }
    if (major === 4) {
      const items = [];
      while (this.buffer[this.offset] !== 0xff) items.push(this.read());
      this.offset += 1;
      return items;
    }
    if (major === 5) {
      const map = new Map();
      while (this.buffer[this.offset] !== 0xff) map.set(this.read(), this.read());
      this.offset += 1;
      return map;
    }
...
function verifyCertificate(child, issuer) {
  const now = Date.now();
  if (now < Date.parse(child.validFrom) || now > Date.parse(child.validTo)) throw new Error('ATTESTATION_CERTIFICATE_EXPIRED');
...
function validateUserData(value, binding) {
  try {
    if (JSON.stringify(canonicalJson(JSON.parse(value.toString('utf8')))) !== JSON.stringify(canonicalJson(binding))) {
      throw new Error('ATTESTATION_BINDING_INVALID');
    }
  } catch (error) {
    if (error.message === 'ATTESTATION_BINDING_INVALID') throw error;
    throw new Error('ATTESTATION_BINDING_INVALID');
  }
}

function verifyNitroAttestation(encoded, release, binding) {
  const sign1 = decodeCbor(Buffer.from(encoded, 'base64url'));
  if (!Array.isArray(sign1) || sign1.length !== 4) throw new Error('ATTESTATION_COSE_INVALID');
  const payload = decodeCbor(asBuffer(sign1[2], 'payload'));
  const certificate = new crypto.X509Certificate(asBuffer(mapValue(payload, 'certificate'), 'certificate'));
  const cabundle = mapValue(payload, 'cabundle');
  ...
  if (!chain[0].raw.equals(root.raw)) throw new Error('ATTESTATION_ROOT_INVALID');
  for (let index = chain.length - 1; index > 0; index -= 1) verifyCertificate(chain[index], chain[index - 1]);
  verifyCertificate(certificate, chain[chain.length - 1]);

  const signedPayload = verifyCoseSign1(sign1, certificate);
