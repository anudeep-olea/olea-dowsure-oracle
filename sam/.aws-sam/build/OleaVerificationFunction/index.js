...
async function issueChallenge(body) {
  ...
  challenge.challengeSignature = `poc:${sha256(challenge)}`;
...
async function submitEvidence(body) {
  if (!body || !body.requestId || !body.nonce || !body.policyVersion || !body.encryptedEvidenceReference || !body.manifestDigest) {
    return response(400, {status: 'REJECTED', reasonCode: 'PAYLOAD_CORRUPTED'});
  }
  ...
  if (body.eifDigest) {
    ...
    if (['pcr0', 'pcr1', 'pcr2'].some((field) => body[field] !== release[field])) {
      return response(422, {status: 'REJECTED', reasonCode: 'PCR_MISMATCH'});
    }
  }
  ...
  try {
    await dynamo.send(new UpdateCommand({
      TableName: process.env.CHALLENGE_TABLE,
      Key: {requestId: body.requestId},
      UpdateExpression: 'SET used = :true',
      ConditionExpression: 'used = :false AND nonce = :nonce',
      ExpressionAttributeValues: {':true': true, ':false': false, ':nonce': body.nonce}
    }));
  } catch (error) {
  ...
  const evidenceId = body.evidenceId || `ev-${crypto.randomUUID()}`;
  const receipt = {
    evidenceId,
    requestId: body.requestId,
    status: 'ACCEPTED',
  ...
  await s3.send(new PutObjectCommand({
    Bucket: process.env.EVIDENCE_VAULT,
    Key: `evidence/${evidenceId}.json`,
